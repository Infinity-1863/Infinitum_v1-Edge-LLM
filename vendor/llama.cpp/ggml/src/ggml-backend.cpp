...
static ggml_backend_sched_moe_copy_cache_entry & ggml_backend_sched_moe_copy_cache_get(
    ...
    if (entry.source != input ||
        ...
        entry.resident_ids.resize(ggml_bitset_size(n_expert));
    }
    return entry;
}
        ...
        for (int input_id = 0; input_id < split->n_inputs; input_id++) {
            ...
            if (input->flags & GGML_TENSOR_FLAG_INPUT) {
                ...
                if (split->graph.n_nodes > 0 &&
                    ...
                    ggml_bitset_t * resident_ids = nullptr;
                    if (ggml_backend_sched_moe_copy_cache_enabled()) {
                        ggml_backend_sched_moe_copy_cache_entry & cache_entry =
                    ...
                    auto copy_experts = [&](int32_t first_id, int32_t last_id) {
                        ...
                        if (resident_ids != nullptr) {
                            for (int32_t expert_id = first_id; expert_id <= last_id; ++expert_id) {
                                ggml_bitset_set(resident_ids, expert_id);
                            }
                        }
                    };
                    ...
                    while (!ggml_bitset_get(used_ids.data(), id) ||
                            (resident_ids != nullptr && ggml_bitset_get(resident_ids, id))) {
                        id++;
                        if (id >= n_expert) {
                            break;
                        }
