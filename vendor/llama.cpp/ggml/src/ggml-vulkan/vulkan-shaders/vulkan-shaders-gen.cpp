...
void process_shaders() {
    ...
    for (const auto& tname : type_names) {
        ...
        string_to_spv("mul_mat_vec_id_" + tname + "_f16_f32", shader, merge_maps(base_dict, {{"MUL_MAT_ID", "1"}, {data_a_key, "1"}, {"B_TYPE", "float16_t"}, {"B_TYPEV2", "f16vec2"}, {"B_TYPEV4", "f16vec4"}, {"D_TYPE", "float"}}));
        ...
        string_to_spv("mul_mat_vec_id_" + tname + "_f16_f32_subgroup", shader, merge_maps(base_dict, {{"MUL_MAT_ID", "1"}, {data_a_key, "1"}, {"B_TYPE", "float16_t"}, {"B_TYPEV2", "f16vec2"}, {"B_TYPEV4", "f16vec4"}, {"D_TYPE", "float"}, {"USE_SUBGROUP_ADD", "1"}}));
        ...
        string_to_spv("mul_mat_vec_id_" + tname + "_f16_f32_subgroup_no_shmem", shader, merge_maps(base_dict, {{"MUL_MAT_ID", "1"}, {data_a_key, "1"}, {"B_TYPE", "float16_t"}, {"B_TYPEV2", "f16vec2"}, {"B_TYPEV4", "f16vec4"}, {"D_TYPE", "float"}, {"USE_SUBGROUP_ADD_NO_SHMEM", "1"}}));
...
void write_output_files() {
    ...
    for (const auto& tname : type_names) {
        ...
        if (btype == "f16" && tname != "mxfp4" && tname != "nvfp4") {
            continue;
        }
