...
static bool ggml_vk_infinitum_debug_enabled() {
    const char * value = std::getenv("GGML_VK_ALLOC_DEBUG");
    return value != nullptr && value[0] != '\0' && value[0] != '0';
}

static void ggml_vk_infinitum_debug_log(const char * fmt, ...) {
    if (!ggml_vk_infinitum_debug_enabled()) {
        return;
    }
    std::fprintf(stderr, "infinitum-vulkan: ");
    va_list args;
    va_start(args, fmt);
...
struct vk_device_struct {
    ...
    vk_pipeline pipeline_dequant_mul_mat_vec_id_f16[DMMV_WG_SIZE_COUNT][GGML_TYPE_COUNT];
...
static const char * hostptr_import_handle_name(vk::ExternalMemoryHandleTypeFlagBits handle_type) {
    switch (handle_type) {
        case vk::ExternalMemoryHandleTypeFlagBits::eHostAllocationEXT:
            return "host_allocation";
        case vk::ExternalMemoryHandleTypeFlagBits::eHostMappedForeignMemoryEXT:
            return "host_mapped_foreign_memory";
        default:
            return "unknown";
    }
}
...
static vk_buffer ggml_vk_create_buffer(vk_device& device, size_t size, const std::initializer_list<vk::MemoryPropertyFlags> & req_flags_list,
    ...
    ggml_vk_infinitum_debug_log("create_buffer begin device=%s size=%llu import=%d", device->name.c_str(), (unsigned long long) size, import_ptr != nullptr ? 1 : 0);
    const bool import_hostptr_report = import_ptr != nullptr && []() {
        const char * value = std::getenv("GGML_VK_HOSTPTR_REPORT");
        return value != nullptr && value[0] != '\0' && value[0] != '0';
    }();
    ...
    if (device->buffer_device_address && import_ptr == nullptr) {
    ...
    if (import_ptr) {
        external_memory_bci.handleTypes =
    ...
    ggml_vk_infinitum_debug_log("create_buffer before createBuffer");
    buf->buffer = device->device.createBuffer(buffer_create_info);
    ggml_vk_infinitum_debug_log("create_buffer after createBuffer");

    vk::MemoryRequirements mem_req = device->device.getBufferMemoryRequirements(buf->buffer);
    ggml_vk_infinitum_debug_log("create_buffer after getBufferMemoryRequirements size=%llu bits=%u", (unsigned long long) mem_req.size, mem_req.memoryTypeBits);
    ...
    if (import_ptr) {
        const std::array<vk::ExternalMemoryHandleTypeFlagBits, 2> hostptr_import_handles = {
            vk::ExternalMemoryHandleTypeFlagBits::eHostAllocationEXT,
            vk::ExternalMemoryHandleTypeFlagBits::eHostMappedForeignMemoryEXT,
        };
        for (const vk::ExternalMemoryHandleTypeFlagBits import_handle_type : hostptr_import_handles) {
            vk::MemoryHostPointerPropertiesEXT host_pointer_props;
            try {
                host_pointer_props = device->device.getMemoryHostPointerPropertiesEXT(import_handle_type, import_ptr);
            } catch (vk::SystemError& e) {
                GGML_LOG_WARN("ggml_vulkan: Failed getMemoryHostPointerPropertiesEXT (%s)\n", e.what());
                if (import_hostptr_report) {
                    std::fprintf(stderr,
                            "ggml_vulkan_hostptr_report: imported=0 reason=host_pointer_properties_error handle=%s ptr=%p size=%zu req_size=%llu mem_bits=%u error=%s device=%s\n",
                            hostptr_import_handle_name(import_handle_type), import_ptr, size,
                            static_cast<unsigned long long>(mem_req.size), mem_req.memoryTypeBits,
                            e.what(), device->name.c_str());
                }
                continue;
            }
            ...
            if (import_hostptr_report) {
                std::fprintf(stderr,
                        "ggml_vulkan_hostptr_report: stage=memory_properties handle=%s ptr=%p size=%zu req_size=%llu host_bits=%u mem_bits=%u memory_type_flags=any device=%s\n",
                        hostptr_import_handle_name(import_handle_type), import_ptr, size, static_cast<unsigned long long>(mem_req.size),
                        host_pointer_props.memoryTypeBits, mem_req.memoryTypeBits, device->name.c_str());
            }

            std::vector<uint32_t> candidate_memory_type_indices;
            for (uint32_t memory_type_idx = 0; memory_type_idx < 32; ++memory_type_idx) {
                ...
                candidate_memory_type_indices.push_back(memory_type_idx);
            }
            if (candidate_memory_type_indices.empty()) {
                GGML_LOG_WARN("ggml_vulkan: Memory type for host allocation not found\n");
                if (import_hostptr_report) {
                    std::fprintf(stderr,
                            "ggml_vulkan_hostptr_report: imported=0 reason=memory_type_not_found handle=%s ptr=%p size=%zu req_size=%llu host_bits=%u mem_bits=%u device=%s\n",
                            hostptr_import_handle_name(import_handle_type), import_ptr, size, static_cast<unsigned long long>(mem_req.size),
                            host_pointer_props.memoryTypeBits, mem_req.memoryTypeBits, device->name.c_str());
                }
                continue;
            ...
            for (const uint32_t memory_type_idx : candidate_memory_type_indices) {
                ...
                try {
                    ...
                    import_info.handleType = import_handle_type;
                    ...
                    if (import_hostptr_report) {
                        std::fprintf(stderr,
                                "ggml_vulkan_hostptr_report: imported=1 reason=ok handle=%s ptr=%p size=%zu req_size=%llu type=%u host_bits=%u mem_bits=%u device=%s\n",
                                hostptr_import_handle_name(import_handle_type), import_ptr, size,
                                static_cast<unsigned long long>(mem_req.size), memory_type_idx,
                                host_pointer_props.memoryTypeBits, mem_req.memoryTypeBits, device->name.c_str());
                    }
                    break;
                } catch (const vk::SystemError& e) {
                    if (import_hostptr_report) {
                        std::fprintf(stderr,
                                "ggml_vulkan_hostptr_report: imported=0 reason=allocate_memory_error handle=%s ptr=%p size=%zu req_size=%llu type=%u host_bits=%u mem_bits=%u error=%s device=%s\n",
                                hostptr_import_handle_name(import_handle_type), import_ptr, size,
                                static_cast<unsigned long long>(mem_req.size), memory_type_idx,
                                host_pointer_props.memoryTypeBits, mem_req.memoryTypeBits, e.what(), device->name.c_str());
                    }
                }
            }
        ...
        for (auto it = req_flags_list.begin(); it != req_flags_list.end(); it++) {
            ...
            for (auto mtype_it = memory_type_indices.begin(); mtype_it != memory_type_indices.end(); mtype_it++) {
                try {
                    ggml_vk_infinitum_debug_log("create_buffer before allocateMemory size=%llu type=%u", (unsigned long long) mem_req.size, *mtype_it);
                    buf->device_memory = device->device.allocateMemory({ mem_req.size, *mtype_it, &mem_flags_info });
                    ggml_vk_infinitum_debug_log("create_buffer after allocateMemory");
                    done = true;
                    break;
                } catch (const vk::SystemError& e) {
                    // loop and retry
    ...
    if (import_ptr) {
        ...
        if (buf->memory_property_flags & vk::MemoryPropertyFlagBits::eHostVisible) {
            ggml_vk_infinitum_debug_log("create_buffer before mapMemory");
            buf->ptr = device->device.mapMemory(buf->device_memory, 0, VK_WHOLE_SIZE);
            ggml_vk_infinitum_debug_log("create_buffer after mapMemory");
        }
    }

    ggml_vk_infinitum_debug_log("create_buffer before bindBufferMemory");
    device->device.bindBufferMemory(buf->buffer, buf->device_memory, 0);
    ggml_vk_infinitum_debug_log("create_buffer after bindBufferMemory");

    buf->device = device;
    buf->size = size;

    if (device->buffer_device_address && import_ptr == nullptr) {
        ggml_vk_infinitum_debug_log("create_buffer before getBufferAddress");
        const vk::BufferDeviceAddressInfo addressInfo(buf->buffer);
        buf->bda_addr = device->device.getBufferAddress(addressInfo);
        ggml_vk_infinitum_debug_log("create_buffer after getBufferAddress");
    }

    ggml_vk_infinitum_debug_log("create_buffer before memory_logger");
    device->memory_logger->log_allocation(buf, size);
    ggml_vk_infinitum_debug_log("create_buffer done size=%llu", (unsigned long long) size);
...
#undef CREATE_MM
    ...
    for (uint32_t w = 0; w < DMMV_WG_SIZE_COUNT; ++w) {
        ...
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f16[w][GGML_TYPE_MXFP4],   "mul_mat_vec_id_mxfp4_f16",   arr_dmmv_id_mxfp4_f16_f32_len[reduc16],   arr_dmmv_id_mxfp4_f16_f32_data[reduc16],   "main", mul_mat_vec_id_num_bindings, sizeof(vk_mat_vec_id_push_constants), {rm_iq, 1, 1}, {wg_size_subgroup16, rm_iq}, 1, true, use_subgroups16, force_subgroup_size16);
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f16[w][GGML_TYPE_NVFP4],   "mul_mat_vec_id_nvfp4_f16",   arr_dmmv_id_nvfp4_f16_f32_len[reduc16],   arr_dmmv_id_nvfp4_f16_f32_data[reduc16],   "main", mul_mat_vec_id_num_bindings, sizeof(vk_mat_vec_id_push_constants), {rm_iq, 1, 1}, {wg_size_subgroup16, rm_iq}, 1, true, use_subgroups16, force_subgroup_size16);
...
static vk_device ggml_vk_get_device(size_t idx) {
    ...
    if (vk_instance.devices[idx] == nullptr) {
        ...
        device->buffer_device_address = vk12_features.bufferDeviceAddress && getenv("GGML_VK_DISABLE_BDA") == nullptr;
#if defined(__ANDROID__)
        if (device->buffer_device_address && getenv("GGML_VK_ANDROID_ENABLE_BDA") == nullptr) {
            ggml_vk_infinitum_debug_log("device %s: disabling buffer device address on Android", device->name.c_str());
...
static vk_pipeline ggml_vk_get_dequantize_mul_mat_vec_id(ggml_backend_vk_context * ctx, ggml_type a_type, ggml_type b_type, uint32_t m, uint32_t k) {
    ...
    if (const char * force_dmmv_id_wg = std::getenv("GGML_VK_DMMV_ID_WG")) {
        if (std::strcmp(force_dmmv_id_wg, "large") == 0 || std::strcmp(force_dmmv_id_wg, "1") == 0) {
            dmmv_wg = DMMV_WG_SIZE_LARGE;
        } else if (std::strcmp(force_dmmv_id_wg, "subgroup") == 0 || std::strcmp(force_dmmv_id_wg, "0") == 0) {
            dmmv_wg = DMMV_WG_SIZE_SUBGROUP;
        }
    }
    ...
    if (b_type == GGML_TYPE_F16) {
        return ctx->device->pipeline_dequant_mul_mat_vec_id_f16[dmmv_wg][a_type];
    }
...
static ggml_backend_buffer_t ggml_backend_vk_buffer_type_alloc_buffer(ggml_backend_buffer_type_t buft, size_t size) {
    ...
    ggml_backend_vk_buffer_type_context * ctx = (ggml_backend_vk_buffer_type_context *) buft->context;
    ggml_vk_infinitum_debug_log("alloc_buffer begin buft=%s size=%llu", ctx->name.c_str(), (unsigned long long) size);

    vk_buffer dev_buffer = nullptr;
    try {
        dev_buffer = ggml_vk_create_buffer_device(ctx->device, size);
    } catch (const vk::SystemError& e) {
        ggml_vk_infinitum_debug_log("alloc_buffer vk::SystemError %s", e.what());
        return nullptr;
    }
    ggml_vk_infinitum_debug_log("alloc_buffer after create_buffer_device");

    ggml_backend_vk_buffer_context * bufctx = new ggml_backend_vk_buffer_context(ctx->device, std::move(dev_buffer), ctx->name);
    ggml_vk_infinitum_debug_log("alloc_buffer after context");
...
static ggml_status ggml_backend_vk_graph_compute(ggml_backend_t backend, ggml_cgraph * cgraph) {
    ...
    const bool fusion_report = std::getenv("GGML_VK_FUSION_REPORT") != nullptr;
    uint64_t report_mul_mat_id_nodes = 0;
    uint64_t report_mul_mat_id_src0_bytes = 0;
    uint64_t report_mul_mat_id_mul = 0;
    uint64_t report_mul_mat_id_add_id = 0;
    uint64_t report_mul_mat_id_add_id_mul = 0;
    uint64_t report_topk_moe = 0;
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        if (cgraph->nodes[i]->op == GGML_OP_MUL_MAT || cgraph->nodes[i]->op == GGML_OP_MUL_MAT_ID) {
            ...
            if (fusion_report && cgraph->nodes[i]->op == GGML_OP_MUL_MAT_ID) {
                ++report_mul_mat_id_nodes;
                report_mul_mat_id_src0_bytes += bytes;
            }
        }
        ...
        if (fusion_report && fusion_string != nullptr) {
            if (std::strcmp(fusion_string, "MUL_MAT_ID_MUL") == 0) {
                ++report_mul_mat_id_mul;
            } else if (std::strcmp(fusion_string, "MUL_MAT_ID_ADD_ID") == 0) {
                ++report_mul_mat_id_add_id;
            } else if (std::strcmp(fusion_string, "MUL_MAT_ID_ADD_ID_MUL") == 0) {
                ++report_mul_mat_id_add_id_mul;
            } else if (std::strncmp(fusion_string, "TOPK_MOE_", 9) == 0) {
                ++report_topk_moe;
            }
        }
    ...
    if (fusion_report) {
        std::fprintf(stderr,
            "ggml_vulkan_fusion_report: nodes=%d mul_mat_id=%llu mul_mat_id_src0_mb=%.1f "
            "mul_mat_id_mul=%llu mul_mat_id_add_id=%llu mul_mat_id_add_id_mul=%llu topk_moe=%llu total_mul_mat_mb=%.1f\n",
            cgraph->n_nodes,
            static_cast<unsigned long long>(report_mul_mat_id_nodes),
            double(report_mul_mat_id_src0_bytes) / (1024.0 * 1024.0),
            static_cast<unsigned long long>(report_mul_mat_id_mul),
            static_cast<unsigned long long>(report_mul_mat_id_add_id),
            static_cast<unsigned long long>(report_mul_mat_id_add_id_mul),
            static_cast<unsigned long long>(report_topk_moe),
            double(total_mul_mat_bytes) / (1024.0 * 1024.0));
    }
...
static void ggml_backend_vk_device_get_props(ggml_backend_dev_t dev, struct ggml_backend_dev_props * props) {
    ...
    props->caps = {
        /* .async                 = */ true,
        /* .host_buffer           = */ true,
        /* .buffer_from_host_ptr  = */ ggml_vk_get_device(ctx->device)->external_memory_host,
        /* .events                = */ true,
    };
}
...
static bool ggml_backend_vk_device_supports_op(ggml_backend_dev_t dev, const ggml_tensor * op) {
    ...
    for (int i = 0; i < GGML_MAX_SRC; i++) {
        if (op->src[i] && !tensor_size_supported(ggml_nbytes(op->src[i]))) {
            if (op->op == GGML_OP_SET_ROWS || op->op == GGML_OP_VIEW || op->op == GGML_OP_NONE) {
                ggml_vk_infinitum_debug_log(
                    "supports_op reject op=%s name=%s src%d=%s src_bytes=%llu op_bytes=%llu max_buffer=%llu max_storage=%llu",
                    ggml_op_name(op->op), op->name, i, op->src[i]->name,
                    (unsigned long long) ggml_nbytes(op->src[i]),
                    (unsigned long long) ggml_nbytes(op),
                    (unsigned long long) device->max_buffer_size,
    ...
    if (!tensor_size_supported(ggml_nbytes(op))) {
        if (op->op == GGML_OP_SET_ROWS || op->op == GGML_OP_VIEW || op->op == GGML_OP_NONE) {
            ggml_vk_infinitum_debug_log(
                "supports_op reject op=%s name=%s op_bytes=%llu max_buffer=%llu max_storage=%llu",
        ...
        case GGML_OP_MUL_MAT_ID:
            {
                ...
                if (op->op == GGML_OP_MUL_MAT_ID) {
                    const bool supports_mat_vec_id =
                        op->src[2]->ne[1] == 1 &&
                        op->src[1]->type == GGML_TYPE_F32 &&
                        device->pipeline_dequant_mul_mat_vec_id_f32[DMMV_WG_SIZE_SUBGROUP][src0_type] != nullptr;
                    if (!supports_mat_vec_id &&
...
static vk_buffer ggml_vk_buffer_from_host_ptr(vk_device & device, void * ptr, size_t size) {
    static const bool infinitum_hostptr_report = []() {
        const char * value = std::getenv("GGML_VK_HOSTPTR_REPORT");
        return value != nullptr && value[0] != '\0' && value[0] != '0';
    }();

    if (!device->external_memory_host) {
        if (infinitum_hostptr_report) {
            std::fprintf(stderr, "ggml_vulkan_hostptr_report: imported=0 reason=external_memory_host_disabled ptr=%p size=%zu device=%s\n",
                    ptr, size, device->name.c_str());
        }
        return {};
    }
    ...
    if (uptr & (device->min_imported_host_pointer_alignment - 1)) {
        if (infinitum_hostptr_report) {
            std::fprintf(stderr, "ggml_vulkan_hostptr_report: imported=0 reason=ptr_alignment ptr=%p size=%zu alignment=%llu device=%s\n",
                    ptr, size, static_cast<unsigned long long>(device->min_imported_host_pointer_alignment), device->name.c_str());
        }
        return {};
    }
    if (size & (device->min_imported_host_pointer_alignment - 1)) {
        if (infinitum_hostptr_report) {
            std::fprintf(stderr, "ggml_vulkan_hostptr_report: imported=0 reason=size_alignment ptr=%p size=%zu alignment=%llu device=%s\n",
                    ptr, size, static_cast<unsigned long long>(device->min_imported_host_pointer_alignment), device->name.c_str());
        }
        return {};
    }
    ...
    try {
        ...
        if (infinitum_hostptr_report) {
            std::fprintf(stderr, "ggml_vulkan_hostptr_report: imported=0 reason=create_buffer_error ptr=%p size=%zu error=%s device=%s\n",
                    ptr, size, e.what(), device->name.c_str());
        }
    }

    if (infinitum_hostptr_report) {
        std::fprintf(stderr, "ggml_vulkan_hostptr_report: imported=%d reason=%s ptr=%p size=%zu alignment=%llu device=%s\n",
                buf ? 1 : 0, buf ? "ok" : "empty_buffer", ptr, size,
                static_cast<unsigned long long>(device->min_imported_host_pointer_alignment), device->name.c_str());
    }
