...
export function useProcessingState(): UseProcessingStateReturn {
	...
	let slotFallbackState = $state<ApiProcessingState | null>(null);
	let slotPollTimer: ReturnType<typeof setInterval> | null = null;
	let lastSlotDecoded = 0;
	let lastSlotTime = 0;
	...
	const processingState = $derived.by(() => {
		...
		return activeProcessingState() ?? slotFallbackState;
	});
	...
	function startMonitoring(): void {
		...
		isMonitoring = true;
		void pollSlots();
		slotPollTimer = setInterval(() => void pollSlots(), 1500);
	}
	...
	async function pollSlots(): Promise<void> {
		try {
			...
			const nextToken = activeSlot.next_token?.[0];
			const decoded = Math.max(0, nextToken?.n_decoded ?? 0);
			const remaining = nextToken?.n_remain ?? 0;
			const now = Date.now();
			let tokensPerSecond = 0;

			if (lastSlotTime > 0 && now > lastSlotTime && decoded >= lastSlotDecoded) {
				tokensPerSecond = ((decoded - lastSlotDecoded) / (now - lastSlotTime)) * 1000;
			}
			lastSlotDecoded = decoded;
			lastSlotTime = now;

			const maxTokens = activeSlot.params?.max_tokens ?? activeSlot.params?.n_predict ?? -1;

			slotFallbackState = {
				status: decoded > 0 ? 'generating' : 'preparing',
				tokensDecoded: decoded,
				tokensRemaining: remaining,
				contextUsed: decoded,
				contextTotal: activeSlot.n_ctx ?? null,
				outputTokensUsed: decoded,
				outputTokensMax: maxTokens,
				temperature: activeSlot.params?.temperature ?? config().temperature ?? 0.8,
				topP: activeSlot.params?.top_p ?? config().top_p ?? 0.95,
				speculative: !!activeSlot.speculative,
				hasNextToken: !!nextToken?.has_next_token,
				tokensPerSecond
			};
		} catch {
			// Best-effort progress fallback only; streaming timings remain authoritative.
		}
	}
