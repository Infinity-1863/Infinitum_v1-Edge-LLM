...
class ChatStore {
	...
	private async streamChatCompletion(
		...
		let streamedReasoningChunks = 0;
		let resolvedModel: string | null = null;
		let modelPersisted = false;
		const convId = assistantMessage.convId;

		const cleanLiveGptOssText = (value: string): string =>
			value
				.replace(/<\|start\|>assistant/g, '')
				.replace(/<\|channel\|>(analysis|final|commentary)?/g, '')
				.replace(/<\|message\|>/g, '')
				.replace(/<\|end\|>/g, '')
				.replace(/<\|return\|>/g, '')
				.replace(/<\|call\|>/g, '')
		...
		const updateStreamingUI = () => {
			const visibleContent = cleanLiveGptOssText(streamedContent);
			this.setChatStreaming(convId, visibleContent, currentMessageId);
			const idx = conversationsStore.findMessageIndex(currentMessageId);
			conversationsStore.updateMessageAtIndex(idx, { content: visibleContent });
		};
		...
		const streamCallbacks: ChatStreamCallbacks = {
			...
			onReasoningChunk: (chunk: string) => {
				...
				streamedReasoningChunks += 1;
				const visibleThinking = cleanLiveGptOssText(streamedContent || streamedReasoningContent);
				// mark streaming state so a stop mid-thinking can persist the partial reasoning
				this.setChatStreaming(convId, visibleThinking, currentMessageId);
				const idx = conversationsStore.findMessageIndex(currentMessageId);
			...
			onAssistantTurnComplete: async (
				...
				const visibleContent = cleanLiveGptOssText(content);
			...
			onFlowComplete: (finalTimings?: ChatMessageTimings) => {
				...
				if (onComplete) onComplete(cleanLiveGptOssText(streamedContent));
				...
				if (config().preEncodeConversation) {
					this.triggerPreEncode(
						allMessages,
						assistantMessage,
						cleanLiveGptOssText(streamedContent),
						effectiveModel,
		...
		await ChatService.sendMessage(
			allMessages,
			{
				...
				onComplete: async (
					...
					const content = cleanLiveGptOssText(streamedContent || finalContent || '');
				...
				contextWithContinue,
				{
					...
					onReasoningChunk: (chunk: string) => {
						...
						const visibleThinking =
							...
							(appendedContent ? '' : `\n\n${appendedReasoning}`);
						// mark streaming state so a stop mid-thinking can persist the partial reasoning
						this.setChatStreaming(msg.convId, visibleThinking, msg.id);
						conversationsStore.updateMessageAtIndex(idx, {
							content: visibleThinking,
