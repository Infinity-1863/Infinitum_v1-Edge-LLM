...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    const uint32_t n_expert_agg = std::min<uint32_t>(n_expert_used, hparams.n_expert_used);
    ...
    // aggregate experts
    // Keep the warmup graph bounded by hparams.n_expert_used, while allowing
    // runtime top-k overrides to aggregate fewer selected experts.
...
ggml_tensor * llm_graph_context::build_attn(
    ...
    // build_attn_mha splits q across k streams. If the target KV has more streams
    // than the active draft batch, slice to the contiguous active stream range.
    if (k->ne[3] > 1 && (uint32_t) k->ne[3] != ubatch.n_seqs_unq) {
        ...
        GGML_ASSERT((uint32_t) (max_s - min_s + 1) == ubatch.n_seqs_unq &&
                "MTP src-kv attention requires active draft seq_ids to be contiguous");
