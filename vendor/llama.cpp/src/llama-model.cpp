...
static bool llama_infinitum_gpu_mmap_hostptr_enabled() {
    const char * value = std::getenv("LLAMA_INFINITUM_GPU_MMAP_HOSTPTR");
    if (value == nullptr || value[0] == '\0') {
        value = std::getenv("LLAMA_INFINITUM_ANDROID_GPU_MMAP_HOSTPTR");
    }
    return value != nullptr && value[0] != '\0' && value[0] != '0';
}

static bool llama_infinitum_model_debug_enabled() {
    const char * value = std::getenv("LLAMA_INFINITUM_DEBUG");
    return value != nullptr && value[0] != '\0' && value[0] != '0';
}

static void llama_infinitum_model_debug_log(const char * fmt, ...) {
    if (!llama_infinitum_model_debug_enabled()) {
        return;
    }
    std::fprintf(stderr, "infinitum-model: ");
    va_list args;
    va_start(args, fmt);
...
bool llama_model_base::load_tensors(llama_model_loader & ml) {
    ...
    // create tensors for the weights
    {
        ...
        llama_infinitum_model_debug_log("load_tensors after load_arch_tensors");
        ...
        llama_infinitum_model_debug_log("load_tensors after generic scales");
        // output scales
    ...
    llama_infinitum_model_debug_log("load_tensors before done_getting_tensors");
    ml.done_getting_tensors();
    llama_infinitum_model_debug_log("load_tensors after done_getting_tensors");
    ...
    llama_infinitum_model_debug_log("load_tensors after tensors_by_name contexts=%zu", ml.ctx_map.size());

    ml.init_mappings(true, use_mlock ? &pimpl->mlock_mmaps : nullptr);
    llama_infinitum_model_debug_log("load_tensors after init_mappings");
    ...
    for (auto & [buft, ctx_ptr] : ml.ctx_map) {
        ...
        for (ggml_tensor * t = ggml_get_first_tensor(ctx); t != nullptr; t = ggml_get_next_tensor(ctx, t)) {
            ...
            if (ctx_tensor_count <= 8) {
                llama_infinitum_model_debug_log(
                        "load_tensors buft=%s tensor[%zu]=%s bytes=%llu type=%s",
                        ggml_backend_buft_name(buft), ctx_tensor_count - 1,
                        ggml_get_name(t), (unsigned long long) ggml_nbytes(t), ggml_type_name(t->type));
            }
        }
        llama_infinitum_model_debug_log(
                "load_tensors allocate buft=%s tensors=%zu tensor_bytes=%llu max_tensor=%llu",
                ggml_backend_buft_name(buft), ctx_tensor_count,
                (unsigned long long) ctx_tensor_bytes,
                (unsigned long long) ggml_get_max_tensor_size(ctx));
        ...
        if ((dev_type == GGML_BACKEND_DEVICE_TYPE_GPU || dev_type == GGML_BACKEND_DEVICE_TYPE_IGPU) &&
                ...
                !llama_infinitum_gpu_mmap_hostptr_enabled()) {
            LLAMA_LOG_INFO("%s: disabling GPU mmap hostptr buffer for %s; using device buffer upload\n",
        ...
        if (ml.use_mmap && use_mmap_buffer && buffer_from_host_ptr_supported && is_default_buft) {
            llama_infinitum_model_debug_log("load_tensors mmap hostptr path buft=%s", ggml_backend_buft_name(buft));
            ...
            for (uint32_t idx = 0; idx < ml.files.size(); idx++) {
                ...
                llama_infinitum_model_debug_log("load_tensors mmap hostptr allocated buft=%s bytes=%llu", ggml_backend_buft_name(buft), (unsigned long long) (last - first));
                bufs.emplace_back(buf);
                buf_map.emplace(idx, buf);
            }
        } else {
            llama_infinitum_model_debug_log("load_tensors alloc ctx tensors path buft=%s", ggml_backend_buft_name(buft));
            ...
            llama_infinitum_model_debug_log("load_tensors allocated buft=%s bytes=%llu", ggml_backend_buft_name(buft), (unsigned long long) ggml_backend_buffer_get_size(buf));
