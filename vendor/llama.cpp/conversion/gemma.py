...
class Gemma4Model(Gemma3Model):
    ...
    @staticmethod
    def _infinitum_external_moe_enabled() -> bool:
        value = os.environ.get("LLAMA_INFINITUM_GEMMA4_EXTERNAL_MOE", "")
    ...
    def filter_tensors(cls, item: tuple[str, Callable[[], Tensor]]) -> tuple[str, Callable[[], Tensor]] | None:
        ...
        if cls._infinitum_external_moe_enabled() and ".experts." in name and name.endswith((
            ".experts.gate_up_proj",
            ".experts.gate_up_proj.weight",
            ".experts.down_proj",
            ".experts.down_proj.weight",
        )):
            logger.info(f"Infinitum external MoE: skipping Gemma4 expert tensor {name}")
            return None
