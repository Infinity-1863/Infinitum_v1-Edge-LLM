...
def infinitum_core_only_enabled() -> bool:
    value = os.environ.get("LLAMA_INFINITUM_GPT_OSS_CORE_ONLY", "")
    return value not in ("", "0")


def infinitum_hot_expert_manifest() -> dict[int, list[int]]:
    path = os.environ.get("LLAMA_INFINITUM_GPT_OSS_HOT_EXPERTS_MANIFEST", "")
    if not path:
        return {}
    with open(path, "r", encoding="utf-8") as input_file:
        data = json.load(input_file)
    selected = data.get("selected_experts_by_layer") or {}
    return {int(layer): [int(expert) for expert in experts] for layer, experts in selected.items()}
...
class GptOssModel(TextModel):
    ...
    def generate_extra_tensors(self) -> Iterable[tuple[str, Tensor]]:
        if infinitum_core_only_enabled():
            logger.info("LLAMA_INFINITUM_GPT_OSS_CORE_ONLY=1: skipping repacked GPT-OSS expert tensors")
            return []

        hot_experts_by_layer = infinitum_hot_expert_manifest()
        ...
        for name, data_torch in self.get_tensors():
            if hot_experts_by_layer and "mlp.experts." in name:
                layer_id = int(name.split(".layers.", 1)[1].split(".", 1)[0])
                experts = hot_experts_by_layer[layer_id]
                data_torch = data_torch[experts]
    ...
    def filter_tensors(cls, item: tuple[str, Callable[[], Tensor]]) -> tuple[str, Callable[[], Tensor]] | None:
        ...
        if infinitum_core_only_enabled() and "mlp.experts." in name:
            return None
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        hot_experts_by_layer = infinitum_hot_expert_manifest()
        if hot_experts_by_layer and bid is not None:
            experts = hot_experts_by_layer.get(int(bid))
    ...
    def set_gguf_parameters(self):
        hot_experts_by_layer = infinitum_hot_expert_manifest()
        if hot_experts_by_layer:
            hot_counts = {len(experts) for experts in hot_experts_by_layer.values()}
            if len(hot_counts) != 1:
                raise ValueError(f"Hot expert manifest must use one expert count for every layer, got {sorted(hot_counts)}")
            hot_count = next(iter(hot_counts))
            self.hparams["num_local_experts"] = hot_count
            self.hparams["num_experts"] = hot_count
            self.hparams["num_experts_per_tok"] = min(int(self.hparams.get("num_experts_per_tok", 4)), hot_count)
            logger.info(f"LLAMA_INFINITUM_GPT_OSS_HOT_EXPERTS_MANIFEST: exporting {hot_count} experts per layer")
